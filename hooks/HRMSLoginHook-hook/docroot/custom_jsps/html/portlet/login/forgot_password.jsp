...
<%
	User user2 = (User) request
			.getAttribute(WebKeys.FORGOT_PASSWORD_REMINDER_USER);
...
<aui:form action="<%=forgotPasswordURL%>" method="post" name="fm">
	...
	<liferay-ui:error exception="<%=NoSuchUserException.class%>"
		message='<%="the-"
						+ TextFormatter.format(authType, TextFormatter.K)
	...
	<aui:fieldset>
		<c:choose>
			<c:when test="<%=user2 == null%>">
				...
				<%
					String loginParameter = null;
									...
									String loginValue = ParamUtil.getString(request,
											loginParameter);
				%>
				...
				<aui:button-row>
					<aui:button type="submit"
						value='<%=PropsValues.USERS_REMINDER_QUERIES_ENABLED ? "next"
										: "send-new-password"%>' />
				</aui:button-row>
			...
			<c:when
				...
				<c:if
					test="<%=Validator.isNotNull(user2
					...
					<div class="alert alert-info">
						<%=LanguageUtil
										.format(pageContext,
												"a-new-password-will-be-sent-to-x-if-you-can-correctly-answer-the-following-question",
												login)%>
					</div>

					<aui:input autoFocus="<%=true%>"
						label="<%=HtmlUtil.escape(user2
										.getReminderQueryQuestion())%>"
				...
				<c:choose>
					<c:when
						test="<%=PropsValues.USERS_REMINDER_QUERIES_REQUIRED
											&& !user2.hasReminderQuery()%>">
						<div class="alert alert-info">
							<liferay-ui:message
					...
					<c:otherwise>
						...
						<aui:button-row>
							<aui:button id="finalClick" name="finalClick" type="submit" value='<%=company
												.isSendPasswordResetLink() ? "send-password-reset-link"
												: "send-new-password"%>'  />
						</aui:button-row>
					</c:otherwise>
				</c:choose>
...
<aui:script>
 ...
 AUI().use('aui-base','liferay-util-window','aui-io-plugin-deprecated',function(A){
    A.one('#<portlet:namespace/>finalClick').on('click', function(event){
		alert('hello world ');  
		
		var x = document.getElementsByClassName("yui3-widget-stdmod")[0].id;
		alert(x);
